# DialoguePro — Branching Dialogue System for Roblox
...
```
DialoguePro/
├── DialoguePro.lua    -- Main facade (require this)
...
├── build-model.cjs       -- Builds DialoguePro.rbxmx from the Lua sources
├── DialoguePro.rbxmx  -- Pre-built Roblox Model (insert into ReplicatedStorage)
├── README.md             -- This file
├── Example/
...
Insert the **DialoguePro** model into `ReplicatedStorage` from the Toolbox, or drag `DialoguePro.rbxmx` directly into Studio. The model includes a `Demo/` folder with ready-to-run example scripts.

**Option B — Manual:**
Place the `DialoguePro` folder in `ReplicatedStorage` with the 6 ModuleScripts inside it.
...
Use `[set key value]` on nodes (fires when entered) or choices (fires when clicked). **Note:** `[set]` does assignment, not arithmetic. `[set gold 500]` sets gold *to* 500, it does not add 500. For math operations (spending/earning currency), use `[call]` with a registered action instead (see below).
...
| Method | Description |
|---|---|
| `DialoguePro.new(themeOverrides?)` | Create new engine instance |
