...
function PropertyPanel:_populateNode(nodeId)
	...
	local btnW = 30
	local contentW = T.Sizes.PanelWidth - 16 - 6
	local speakerField = Instance.new("TextBox", self._scroll)
	speakerField.Size = UDim2.fromOffset(contentW - btnW - 4, T.Sizes.FieldHeight)
	speakerField.Position = UDim2.fromOffset(8, y)
	speakerField.BackgroundColor3 = T.Colors.Field
	speakerField.BorderSizePixel = 0
	speakerField.TextColor3 = T.Colors.Text
	speakerField.PlaceholderText = "NPC model name"
	speakerField.PlaceholderColor3 = T.Colors.TextPlaceholder
	speakerField.TextSize = T.Sizes.Text
	speakerField.Font = T.Fonts.Default
	speakerField.TextXAlignment = Enum.TextXAlignment.Left
	speakerField.ClearTextOnFocus = false
	speakerField.Text = node.speaker or ""
	speakerField.Active = true
	Instance.new("UICorner", speakerField).CornerRadius = UDim.new(0, 4)
	Instance.new("UIStroke", speakerField).Color = T.Colors.FieldBorder

	local dropBtn = Instance.new("TextButton", self._scroll)
	dropBtn.Size = UDim2.fromOffset(btnW, T.Sizes.FieldHeight)
	dropBtn.Position = UDim2.fromOffset(8 + contentW - btnW, y)
	dropBtn.BackgroundColor3 = T.Colors.Accent
	dropBtn.TextColor3 = Color3.fromRGB(255, 255, 255)
	dropBtn.Text = "v"
	dropBtn.TextSize = T.Sizes.Text
	dropBtn.Font = T.Fonts.Bold
	...
	local dropList = Instance.new("ScrollingFrame", self._scroll)
	dropList.Name = "SpeakerDropdown"
	dropList.Size = UDim2.new(1, -16, 0, 0)
	dropList.Position = UDim2.new(0, 8, 0, y)
	dropList.BackgroundColor3 = Color3.fromRGB(38, 38, 42)
	dropList.BorderSizePixel = 0
	dropList.Visible = false
	dropList.ZIndex = 10
	dropList.ScrollBarThickness = 4
	dropList.ScrollBarImageColor3 = Color3.fromRGB(70, 70, 70)
	Instance.new("UICorner", dropList).CornerRadius = UDim.new(0, 4)
	Instance.new("UIStroke", dropList).Color = T.Colors.Accent
	...
	local function refreshDropdown()
		...
		if #names == 0 then
			table.insert(names, "(no NPC models found)")
		end

		local itemH = 22
		for i, name in ipairs(names) do
			local item = Instance.new("TextButton", dropList)
			item.Size = UDim2.new(1, -4, 0, itemH)
			item.Position = UDim2.fromOffset(0, (i - 1) * itemH)
			item.BackgroundColor3 = Color3.fromRGB(38, 38, 42)
			item.BackgroundTransparency = 0
			item.TextColor3 = T.Colors.Text
			item.Text = "  " .. name
			item.TextSize = T.Sizes.SmallText
			item.Font = T.Fonts.Default
			item.TextXAlignment = Enum.TextXAlignment.Left
			item.BorderSizePixel = 0
			item.ZIndex = 11

			item.MouseEnter:Connect(function()
				item.BackgroundColor3 = T.Colors.Accent
			end)
			item.MouseLeave:Connect(function()
				item.BackgroundColor3 = Color3.fromRGB(38, 38, 42)
			end)

			if name ~= "(no NPC models found)" then
				table.insert(self._conns, item.MouseButton1Click:Connect(function()
					speakerField.Text = name
					local val = name ~= "" and name or nil
		...
		local totalH = math.min(#names * itemH, 6 * itemH)
		dropList.Size = UDim2.new(1, -16, 0, totalH)
		dropList.CanvasSize = UDim2.fromOffset(0, #names * itemH)
	end

	table.insert(self._conns, dropBtn.MouseButton1Click:Connect(function()
