...
function NodeWidget:_build()
	...
	local dragH = Instance.new("TextButton", frame)
...
function NodeWidget:GetChoiceDotCenter(i)
	...
	local spacing = Theme.Sizes.NodeWidth / (#choices + 1)
	local frameH = self._frame.Size.Y.Offset
	return Vector2.new(
...
function NodeWidget:SetConnecting(active)
	local dot = self._inputDot
	if not dot then return end
	local T = Theme
	if active then
		dot.Size = UDim2.fromOffset(T.Sizes.DotRadius * 4, T.Sizes.DotRadius * 4)
		dot.Position = UDim2.new(0.5, -T.Sizes.DotRadius * 2, 0, -T.Sizes.DotRadius * 2)
		local glow = dot:FindFirstChild("Glow") or Instance.new("UIStroke", dot)
		glow.Name = "Glow"
		glow.Color = T.Colors.InputDot
		glow.Thickness = 2
		glow.Transparency = 0.3
	else
		dot.Size = UDim2.fromOffset(T.Sizes.DotRadius * 2, T.Sizes.DotRadius * 2)
		dot.Position = UDim2.new(0.5, -T.Sizes.DotRadius, 0, -T.Sizes.DotRadius)
