# DialoguePro — Complete Documentation
...
Place the `DialoguePro` folder inside `ReplicatedStorage` in Roblox Studio. Your Explorer should look like this:
...
local DialoguePro = require(ReplicatedStorage.DialoguePro.DialoguePro)
local DialogueParser = require(ReplicatedStorage.DialoguePro.DialogueParser)
...
```lua
local Theme = require(ReplicatedStorage.DialoguePro.Theme)
local engine = DialoguePro.new(Theme.Presets.Fantasy)
```
...
Pass a table of overrides to `DialoguePro.new()`:
...
Require it from `ReplicatedStorage.DialoguePro.DialoguePro`.

---

#### `DialoguePro.new(themeOverrides?)`
...
local engine = DialoguePro.new({ typewriterSpeed = 0.05 })
...
> **Singleton behavior:** Only one dialogue can be active at a time per client. Calling `DialoguePro.new()` automatically closes any previously active dialogue instance. Create one engine for the whole game and reuse it across all NPCs — see [Can I run multiple dialogue engines at once?](#can-i-run-multiple-dialogue-engines-at-once) for the recommended pattern.
...
Require it from `ReplicatedStorage.DialoguePro.DialogueParser`.
...
Require it from `ReplicatedStorage.DialoguePro.Theme`.
...
```lua
local Theme = require(ReplicatedStorage.DialoguePro.Theme)
local engine = DialoguePro.new(Theme.Presets.SciFi)
```
...
local engine = DialoguePro.new(myTheme)
```
...
Also check the output window for warnings — the engine prints `[DialoguePro] Tree not found: <id>` if the tree ID doesn't exist.
...
**Cause:** `DialoguePro.new()` closes any previously running dialogue when called. If you call `new()` inside an NPC prompt callback, each NPC creates a new engine that closes the previous one.
...
**Short answer:** No. Only one dialogue engine instance is active at a time per client. Creating a new `DialoguePro.new()` automatically closes the previous active instance.
