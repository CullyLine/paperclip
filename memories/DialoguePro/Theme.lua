--[[
    Theme.lua
    Built-in themes and merge utility for the DialoguePro.

    Usage:
      DialoguePro.new()                          -- uses "Dark" (default)
      DialoguePro.new(Theme.Presets.Fantasy)      -- use a preset
      DialoguePro.new({ textSize = 22 })          -- override individual fields
      DialoguePro.new(Theme.Extend("Bubblegum", { textSize = 22 }))
                                                     -- preset + overrides
]]
...
function Theme.Extend(presetName: string, overrides: { [string]: any }?): { [string]: any }
    ...
    if not preset then
        warn("[DialoguePro] Unknown theme preset: " .. tostring(presetName) .. ". Using default.")
        preset = {}
    end
