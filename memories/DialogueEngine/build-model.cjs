#!/usr/bin/env node
// Generates DialogueEngine.rbxmx — a Roblox Model containing the full
// DialogueEngine runtime ready to insert into ReplicatedStorage.
//
// Run:  node build-model.cjs
//
// Output structure inside the model:
//   DialogueEngine (Folder)
...
const runtimeModules = [
	{ name: "DialogueEngine", file: "DialogueEngine.lua" },
...
const readmeText = `--[[
    ╔══════════════════════════════════════════════════════════════╗
    ║                    DIALOGUE ENGINE                          ║
    ║              Text-based dialogue system for Roblox          ║
    ╚══════════════════════════════════════════════════════════════╝

    SETUP
    ─────
    1. Drag the entire "DialogueEngine" folder into ReplicatedStorage.
...
const xml = `<roblox version="4">
	...
	<Item class="Folder" referent="${rootRef}">
		<Properties>
			<string name="Name">DialogueEngine</string>
		</Properties>
${moduleItems.join("\n")}${demoFolder}
	</Item>
</roblox>
...
const outPath = path.join(DIR, "DialogueEngine.rbxmx");
...
console.log(`\nTo use: drag DialogueEngine.rbxmx into ReplicatedStorage in Roblox Studio`);
