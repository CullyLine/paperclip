...
function DialogueEngine.new(themeOverrides: { [string]: any }?)
    if instance then
        instance:Close()
    end
...
function DialogueEngine:RegisterTree(treeId: string, dialogueData: any)
    self._runner:RegisterTree(treeId, dialogueData)
end

function DialogueEngine:RegisterCondition(callback: (key: string, value: any) -> boolean)
    self._runner:RegisterConditionCallback(callback)
end

function DialogueEngine:_initBuiltInCondition()
...
function DialogueEngine:RegisterVariable(resolver: (variableName: string) -> any?)
    table.insert(self._variableResolvers, resolver)
end

function DialogueEngine:SetFlag(key: string, value: any)
    self._flags[key] = value
end

function DialogueEngine:GetFlag(key: string): any
    return self._flags[key]
end

function DialogueEngine:GetFlags(): { [string]: any }
    return self._flags
end

function DialogueEngine:RegisterAction(name: string, callback: () -> ())
    self._actions[name] = callback
end

function DialogueEngine:_processActions(actions: { any }?)
    ...
    for _, action in ipairs(actions) do
        ...
        elseif action.type == "call" then
            ...
            if fn then
                ...
                warn("[DialogueEngine] Action not found: " .. tostring(action.key))
            end
        end
    end
end

function DialogueEngine:_resolveVariables(text: string): string
...
function DialogueEngine:Start(treeId: string, npcModelOrCallbacks: (Model | any)?, callbacks: any?)
    ...
    if not startNode then
        warn("[DialogueEngine] Tree not found: " .. treeId)
        return
    end
...
function DialogueEngine:StartFromState(treeId: string, savedState: any, npcModelOrCallbacks: (Model | any)?, callbacks: any?)
    local npcModel: Model? = nil
    local cbs: any
    ...
    if not nodeId then
        warn("[DialogueEngine] Could not load state for tree: " .. treeId)
...
function DialogueEngine:_displayNode(nodeId: string)
    if not self._active then return end

    local node = self._runner:GetNode(self._currentTreeId, nodeId)
    if not node then
        warn("[DialogueEngine] Node not found: " .. nodeId)
        self:Close()
        return
    end
...
function DialogueEngine:_endDialogue()
...
function DialogueEngine:Skip()
    if self._ui then
        self._ui:SkipTypewriter()
    end
end

function DialogueEngine:Advance()
    if self._ui then
        self._ui:Advance()
    end
end

function DialogueEngine:IsActive(): boolean
    return self._active
end

function DialogueEngine:Close()
...
function DialogueEngine:SetTheme(themeOverrides: { [string]: any })
    if self._ui then
        self._ui:SetTheme(themeOverrides)
    end
end

function DialogueEngine:SaveState(treeId: string?): any?
...
function DialogueEngine:LoadState(treeId: string, savedState: any): boolean
    local nodeId = self._runner:LoadState(treeId, savedState)
    return nodeId ~= nil
end

function DialogueEngine:GetVisitedNodes(treeId: string): { [string]: boolean }
